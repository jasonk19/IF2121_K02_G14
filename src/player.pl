...
(fishexp(0, 10, 1)).
(ranchexp(0, 10, 1)).
player(unknown, 1, 1, 0, 1, 0, 1, 0, 0, 1000).
...
character(ID) :-
  ID =:= 0 -> write('You choose Fisherman, let\'s start fishing'),nl, 
...
addExp(X) :-
  ...
  (X =:= 0 -> 
   ...
   (NewExp >= Total ->
    format('Level up! ~n', []),
    NewExp2 is NewExp-Total, NewLvl is Level + 1, NewTotal is (100 * NewLvl),
...
addFarmExp(X) :-
  ...
  (NewExp >= Total ->
    format('Level up! ~n', []),
    NewExp2 is NewExp-Total, NewLvl is FarmLevel + 1, NewTotal is (10 * NewLvl),
...
addFishExp(X) :-
  ...
  (NewExp >= Total ->
    format('Level up! ~n', []),
    NewExp2 is NewExp-Total, NewLvl is FishLevel + 1, NewTotal is (10 * NewLvl),
...
addRanchExp(X) :-
  ...
  (NewExp >= Total ->
    format('Level up! ~n', []),
    NewExp2 is NewExp-Total, NewLvl is RanchLevel + 1, NewTotal is (10 * NewLvl),
